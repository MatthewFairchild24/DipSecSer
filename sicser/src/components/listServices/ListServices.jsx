...
export default function ListServices() {
	...
	useEffect(() => {
		const fetchServices = async () => {
			try {
				const response = await fetch('https://localhost:7263/api/Services') //
				if (!response.ok) {
	...
	const HandleDelete = async (id) => {
		if (window.confirm('Вы уверены, что хотите удалить этот сервис?'))
			try {
				await fetch(`https://localhost:7263/api/Services/${id}`, {
					method: 'DELETE',
				})
	...
	const handleSave = async (editedService) => {
		try {
			const response = await fetch(
				`https://localhost:7263/api/Services/${editedService.id}`,
				{
					method: 'PUT',
					headers: {
	...
	const handleAdd = async (newService) => {
		try {
			const response = await fetch(
				'https://localhost:7263/api/Services/uploadService',
				{
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
